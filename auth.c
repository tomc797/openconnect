		...
		xmlNodePtr node, int *cert_rq)
{
	static const struct {
		...
		{ XCAST("sha256"), CERT_AUTH_CHAL_SHA256 },
		{ XCAST("sha384"), CERT_AUTH_CHAL_SHA384 },
		{ XCAST("sha512"), CERT_AUTH_CHAL_SHA512 },
		{ NULL },
	};
	xmlNodePtr child;
	xmlChar *content;
	...
	*cert_rq = CERT_AUTH_REQ_CERT1|CERT_AUTH_REQ_CERT2;
	/* xml_node is a multiple-client-cert-request element */
	for (child = node->children; child; child = child->next) {
		...
		for (entry = algos; entry->name; entry++) {
			if (xmlStrcasecmp(entry->name, content) == 0) {
				*cert_rq |= entry->value;
				break;
			}
		}
		...
		int cert_rq, const char *challenge, struct oc_text_buf *body)
{
	static const struct {
		...
		{ CERT_AUTH_CHAL_SHA256, XCAST("sha256") },
		{ CERT_AUTH_CHAL_SHA384, XCAST("sha384") },
		{ CERT_AUTH_CHAL_SHA512, XCAST("sha512") },
		{ 0, XCAST("(unknown)") },
	};
	xmlDocPtr doc;
	...
	if ((cert_rq & CERT_AUTH_CHAL_MASK) == 0) {
		vpn_progress(vpninfo, PRG_ERR,
	...
	for (entry = algos; entry->type != 0; entry++) {
		if (resp.type == entry->type)
			break;
	}
 ...
 bad:
	free(identity); free(resp.data);
	if (doc) xmlpost_complete(doc, NULL);
	return -ENOMEM;
}
