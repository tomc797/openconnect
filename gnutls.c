				   ...
				   gnutls_x509_crl_t *crl)
{
	...
	while ((err = gnutls_pkcs12_verify_mac(p12, pass)) == GNUTLS_E_MAC_VERIFY_FAILED) {
		...
		if (!pass) {
			...
			if (err == 0 && (pass = strdup("")) == NULL)
				err = GNUTLS_E_MEMORY_ERROR;
			if (err != GNUTLS_E_MAC_VERIFY_FAILED)
				break;
		} else if (pass[0] == 0) {
			err = gnutls_pkcs12_verify_mac(p12, NULL);
			if (err != GNUTLS_E_MAC_VERIFY_FAILED) {
				free_pass(&pass);
				break;
			}
...
{
	gnutls_pcert_st *pcerts = calloc(nr_certs, sizeof pcerts[0]);
	unsigned int i;
	int err;
...
static int load_trust(struct openconnect_info *vpninfo)
{
	...
	if (vpninfo->cafile) {
		...
		ret = gnutls_x509_trust_list_add_trust_mem(tlist, &tmp, NULL,
		    GNUTLS_X509_FMT_PEM,
		    GNUTLS_TL_NO_DUPLICATES | GNUTLS_TL_USE_IN_TLS,
		    0);
		if (ret < 0) {
			vpn_progress(vpninfo, PRG_ERR,
			    _("Error loading CAs: (%d) %s\n"),
			    ret, gnutls_strerror(ret));
			ret = -EINVAL;
		} else {
			vpn_progress(vpninfo, PRG_INFO,
			    _("Loaded CAs (%d CAs available)\n"),
			    ret);
		}

#define PEM_CRL "-----BEGIN X509 CRL-----"
		if (ret >= 0 && strstr((char *) tmp.data, PEM_CRL)) {

#ifndef GNUTLS_TL_FAIL_ON_INVALID_CRL
#  define GNUTLS_TL_FAIL_ON_INVALID_CRL 0U
#endif
			ret = gnutls_x509_trust_list_add_trust_mem(tlist, NULL, &tmp,
			    GNUTLS_X509_FMT_PEM,
			    GNUTLS_TL_NO_DUPLICATES | GNUTLS_TL_VERIFY_CRL
			    | GNUTLS_TL_FAIL_ON_INVALID_CRL,
			    0);
			if (ret < 0) {
				vpn_progress(vpninfo, PRG_ERR,
				    _("Failed to load CRLs: (%d) %s\n"),
				    ret, gnutls_strerror(ret));
				ret = -EINVAL;
			} else {
				vpn_progress(vpninfo, PRG_INFO,
				    _("Loaded CRLs (%d CRLs available)\n"),
				    ret);
			}
		}
		free(tmp.data);
		...
		gnutls_x509_crl_t *crl_)
{
 ...
 got_key:
	...
	while (1) {
		...
		} else if ((err = gnutls_certificate_get_issuer(vpninfo->https_cred,
			last_cert, &issuer, GNUTLS_TL_GET_COPY)) == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE) {
			/**
			 * Issuer is not in the system trust. Let's try pkcs11
			 */
#ifdef HAVE_P11KIT
			if (cert_is_p11) {
		...
		struct challenge_response *response)
{
	...
	*response =
	  (struct challenge_response) {cert_rq, (char *)sigpem.data};
